...
class DataPreprocessor:
    ...
    def preprocess(self, data, is_training=True):
        ...
        # Se estivermos no modo de treinamento, armazene as colunas
        if is_training:
            self.train_columns = processed_data.columns
        else:
            # Verifique se train_columns foi definido
            ...
            # Se estivermos no modo de previsão, ajuste as colunas para corresponder às colunas de treinamento
            missing_cols = set(self.train_columns) - set(processed_data.columns)
            
            # Adicione as colunas ausentes ao DataFrame
